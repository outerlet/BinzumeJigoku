...
/**
 * XMLをパースして得られたコンテンツデータは一旦DBに保存されるが、そのテーブルの列を示す列挙値<br />
 * データは数百行程度しかないので複数のテーブルを使ってリレーションを組むほどではないし、
 * 組むと逆にパフォーマンスの劣化やコードの煩雑化に繋がるという判断から、本アプリでは1つのテーブルしか使用していない
 */
